...
auto cmp = [](int d, bool E){ return (!(d&1)&&E)||((d&1)&&!E); };

int count(bool B){
	int l,r,c=0; in,l,r;
	int d1 = log2(l), d2 = log2(r);
	if(d1 > d2) swap(d1,d2),swap(l,r); // d2 is greater depth
	while(d2 > d1){
		if(cmp(d2,B))c++;
		d2--,r>>=1; 
	}
