...
float solve1(float t1, float t2, float d){

}

float solve2(float t1, float t2, float d){
...
int main(void){BOOST
	...
	while(t--){
		float t1, t2, d;
		cin >> t1 >> t2 >> d;
		cout << fixed << setprecision(1) << solve1(t1, t2, d) << " ";
	        cout << fixed << setprecision(1) << solve2(t1, t2, d) << endl;
	}
	print_time("Time: ");
	return 0;
}
