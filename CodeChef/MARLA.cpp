...
int aa[1001][1001],n,m;
bool visited[1001][1001]={};

int explore(int s, int i, int j){
	if(i>=n||j>=m||i<0||j<0) return 0;	
	if(visited[i][j]) return 0;
	if(aa[i][j] != s) return 0;
	visited[i][j]=true;
	return 1+explore(s,i+1,j)+explore(s,i-1,j)+explore(s,i,j+1)+explore(s,i,j-1);
}

auto cmp = [](PII & p, int s, int q){ if(q>p.S||(q==p.S&&s < p.F))p=MP(s,q); };
void solve(){
	PII ans=MP(1e9, 0);
	for(int i=0;i<n;i++){
		for(int j=0;j<m;j++){
			if(!visited[i][j]){
				cmp(ans, aa[i][j], explore(aa[i][j], i,j));
			}
		}
	}
	out,ans.F,' ',ans.S,NL;
...
#endif
	in,n,m;
	for(int i=0;i<n;i++)for(int j=0;j<m;j++)in,aa[i][j];
	solve();
	return 0;
}
