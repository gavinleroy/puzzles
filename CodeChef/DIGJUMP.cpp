...
int bfs(int s, int t, int n){
	bool visited[100001]={},dig[10]={};
	queue<PII > qq; qq.push(MP(s, 0));	
	while(!qq.empty()){
		int i = qq.front().F, d = qq.front().S; qq.pop();
		// To optimize this should be tested when adding the items
		// to the queue.
		if(i==t) return d;
		if(visited[i]) continue; // How did this get in here
		visited[i]=true;
		if(!dig[aa[i]-'0']){ // If this type of digit hasn't been added before add all its neighbors
			dig[aa[i]-'0']=true;
			for(int j=0;j<gg[aa[i]-'0'].size();j++)
				if(!visited[gg[aa[i]-'0'][j]]) // This should always be true
					qq.push(MP(gg[aa[i]-'0'][j], d+1));
		}
		if(i+1 < n && !visited[i+1] && !dig[aa[i+1]-'0']) qq.push(MP(i+1, d+1));
		if(i-1 >= 0 && !visited[i-1] && !dig[aa[i-1]-'0']) qq.push(MP(i-1, d+1));
	}
	throw; // There should always be a path from start -> end
}

static I void solve(int n){ out,bfs(0, n-1, n),NL; }
...
int main(){
	...
	in,aa;
	int n=0;
	for(int i=0;aa[i];i++,n++) 
		gg[aa[i]-'0'].push_back(i);	
	solve(n);
	return 0;
}
