...
#define F first
#define S second
#define MKEY(i,j,k,g) ((((((i*37)+j)*37)+k)*37)+g)
...
bool solve(int i, int c1, int c2, int c3){
	if(i == n && c1 >= 0 && c2 >= 0 && c3 >= 0) return true;
	int k = MKEY(i,c1,c2,c3);
	if(dp.find(k) != dp.end()) return dp[k];
	bool m1=false, m2=false;
	if(aa[i] == 5){
		m1 = solve(i+1, c1+1, c2, c3);
	}else if(aa[i] == 10){
		if(c1 > 0) m1 = solve(i+1, c1-1,c2+1,c3);
	}else if(aa[i] == 15){
		if(c1 > 1) m1 = solve(i+1, c1-2, c2, c3+1);
		if(c2 > 0) m2 = solve(i+1, c1, c2-1, c3+1);
	}else throw;
	return dp[k] = m1 || m2;
}
