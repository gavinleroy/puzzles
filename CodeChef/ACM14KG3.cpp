...
vector<char> mm[26];
bool visited[26]={};
char ss[1002],tt[1002],map[10];

static I void cleanup(){ for(int i=0;i<26;i++)mm[i].clear(); }

bool find(int s, int t){
	visited[s]=true;
	bool found=false;
	for(int i=0;i<mm[s].size()&&!found;i++){
		if(mm[s][i]==t) found=true;
		else if(!visited[mm[s][i]]) found=find(mm[s][i],t);
	}
	return found;
}
...
bool solve(){
	...
	for(i=0;ss[i]&&tt[i];i++){
		...
		if(ss[i]!=tt[i] && !find(ss[i]-'a', tt[i]-'a')) return false;
	}
	return !((!ss[i] && tt[i]) || (ss[i] && !tt[i]));
}
...
int main(){
	...
	for(int T=in,m;T--;){
		...
		for(int i=0,u,v;i<m;i++){
			in,map; u=map[0]-'a', v=map[3]-'a';
			assert(0<=u&&u<26&&0<=v&&v<26);
			mm[u].push_back(v);	
		}
		out,solve()?"YES\n":"NO\n";
		cleanup();
