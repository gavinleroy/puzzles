...
int F[maxn];
int bpow(int x, int n) { return n ? n % 2 ? 1LL * x * bpow(x, n - 1) % mod : bpow(1LL * x * x % mod, n / 2) : 1; }
int inv(int x) { return bpow(x, mod - 2); }
int fact(int n) { return n ? F[n] ? F[n] : F[n] = 1LL * n * fact(n - 1) % mod : 1; }
int rfact(int n) { return inv(fact(n)); }
int nCr(int n, int r) { return 1LL * fact(n) * rfact(r) % mod * rfact(n - r) % mod; }
...
int main(){
	...
	int lo,mo,_size,ans;
	for(int T=in;T--;){
		ss1[0]=ss1[1]=ss2[0]=ss2[1]=ans=0; 
		in,_size;
		for(int i=0;i<_size;i++){ in,c; ss1[c-'0']++; }
		for(int i=0;i<_size;i++){ in,c; ss2[c-'0']++; }
		lo = ::abs(ss1[1]-ss2[1]);
		mo = ss1[1] + ss2[1] - 2ll * ::max(ss1[1] + ss2[1] - _size, 0);
		for(int j=lo;j<=mo;j+=2) ans = (ans + nCr(_size,j)) % MOD;
		out,ans,NL;
	}
	return 0;
}
