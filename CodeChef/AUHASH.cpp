...
ll solve(int i, int l, int k){
	if(i > MAXL || l <= 0 || k <= 0) return (!k&&!l)?1ll:0ll;
	if(dp[i][l][k] != -1) return dp[i][l][k];
	// If we can't take this element then we can't take any following it
	if(k - i < 0)  return dp[i][l][k] = 0; 
	ll m1 = solve(i + 1, l - 1, k - i); // Take element
	ll m2 = solve(i + 1, l, k); // Exclude element
	return dp[i][l][k] = m1 + m2;
}
...
int main(){
	...
	for(int T=in,c=1,l,s;T--;c++){
		in,l,s;
		out,"Case ",c,": ";
		if(l > MAXL || s > MAXS) out,0,NL;
		else out,solve(1, l, s),NL;
	}
	return 0;
}
