...
int solve(int v, int p, bool & c){
	...
	// If one of the lower branches was cut then we don't need to 
	// cut any of the current sums away. 
	// If it wasn't cut, but cutting is still optimal, then we can set c=true.
	if(cut || s-1>0){
		c = true;
	       	return s-!cut;
	}
	// Else cutting is not optimal here
	return 1;
...
void solve(){
	bool c=false;
	int a = solve(1,1,c); // Rooted at 1
	if(c) out,a,NL; // If it was cut then just print answer
	// Means it wasn't cut so output max of cutting or leaving
	else out,::max(1, a-1),NL;
}
