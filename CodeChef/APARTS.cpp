...
int aa[1001][1001],n,m,dp[1001][1001];

void solve(){
	for(int i=0;i<n;i++) // Initialize and Read
		for(int j=0;j<m;j++) 
			{ in,aa[i][j]; dp[i][j]=aa[i][j]; }

	for(int i=0;i<n;i++){
		for(int j=0;j<m;j++){
			out,(dp[i][j]>aa[i][j]?0:1);
			if(i==n-1) continue;
			if(j-1>=0) dp[i+1][j-1]=::max(dp[i+1][j-1],dp[i][j]);
