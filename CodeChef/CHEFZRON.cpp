...
int solve(){
	dp[0]=0,dp[1]=1e9,dp[2]=pos[1]-pos[0];
	for(int i=2;i<pos.size();i++)dp[i+1]=::min(pos[i]-pos[i-1]+dp[i-1],pos[i]-pos[i-2]+dp[i-2]);
	return dp[pos.size()];
}
...
int main(){
	...
	for(int T=in,c;T--;){
		in,n; c = 0; pos.clear();
		for(int i=0;i<n;i++){ in,aa[i];	if(aa[i]) pos.pb(i); }
		if(pos.size() == 0) out,0,NL;
		else if(pos.size() == 1) out,-1,NL;
		else{
			int ans=1e9;
			for(int c=0;c<3;c++){
				ans=::min(ans, solve());	
				vector<int> vv(pos);
				pos[0]=0; int diff = n-pos.back();
				for(int i=0;i<pos.size()-1;i++)pos[i+1]=vv[i]+diff;
			}
			out,ans,NL;
		}
	}
