...
int n,k,aa[200001];
bool visited[200001];
struct rot{ int p1,p2,p3; }; 

int followr(int a, int b){
	visited[b]=true;
	if(a==b) return 0;
	else return 1+followr(a, aa[b]);
}

int follow(int i){
	return 1+followr(i, aa[i]);
}

void solve(queue<int> & ee){
	int ind=1,count=0,csize;
	bool done=false;
	vector<rot> vv;
	// Make all even cycles odd
	while(!ee.empty() && count<=k){
		rot r; 
		r.p1=ee.front(); ee.pop();
		r.p2=aa[r.p1];
		r.p3=ee.front();ee.pop();
		swap(aa[r.p1], aa[r.p2]);
		swap(aa[r.p1], aa[r.p3]);
		vv.push_back(r);
		count++;
	}
	// Solve for all odd cycles
	while(!done && count<=k){
		for(int i=ind;i<=n;i++){
			if(aa[i]!=i){
				rot r; 
	...
	if(vv.size()<=k && done){
		...
		for(int i=0;i<vv.size();i++) 
			out,vv[i].p1,' ',vv[i].p2,' ',vv[i].p3,NL;
	}else out,-1,NL;
}
...
#endif
	for(int T=in,csize;T--;){
		in,n,k;	
		queue<int> evens;
		for(int i=1;i<=n;i++) in,aa[i],visited[i]=false;
		// Precompute # of even cycles
		for(int i=1;i<=n;i++){
			if(!visited[i] && aa[i]!=i){
			       	csize = follow(i);
				if(!(csize&1)) evens.push(i);
			}
		}
		// if # of even cycles is odd we can't solve print -1
		if((evens.size())&1) out,-1,NL;
		// else solve by first pairing all even cycles,
		// then do one final sweep and solve all resulting odd cycles
		else solve(evens);
	}
	return 0;
}
