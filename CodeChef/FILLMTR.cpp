...
bool explore(int v, int d, int p, int * dd){
	if(dd[v] != -1) return d == dd[v];
	dd[v] = d;
	bool ret = true;
	for(int i=0;i<G[v].size();i++)
		if(G[v][i].F != p)
			ret = ret && explore(G[v][i].F, (d+G[v][i].S)%2, v, dd);
	return ret;
}

void solve(){
	int color[n+1]; memset(color, -1, sizeof color);
	bool ans = true;
	for(int i=1;i<=n;i++)
		if(color[i]==-1) 
			ans = ans && explore(i, 0, i, (int*)color);
	out,(ans ? "yes\n" : "no\n");
}
