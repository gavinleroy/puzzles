...
ll dfs(int v, bool * done, vector<int> & vv, ll d=1){
	done[v]=true;
	vv.push_back(v);
	ll ret=d;
	for(int i=0;i<G[v].size();i++)
		if(!done[G[v][i]]) 
			ret += dfs(G[v][i], done, vv, d+1);
	return ret;
}
...
void solve(){
	...
	ll ANS[2]={};
	for(int i=1,c;i<=n;i++){
		if(!done[i]){ // Find the clump
			vector<int> vv;
			dfs(i, (bool*)done, vv); 
			c = vv.size();
			sort(vv.begin(), vv.end());
			for(int j=0;j<c;j++)done[vv[j]]=false;
			ANS[c%2] += dfs(((c%2)?vv[c-1]:vv[0]), (bool*)done, vv);
		}
	}
	out,ANS[0],' ',ANS[1],NL;
}
