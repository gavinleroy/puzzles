...
ll solveutil(int i, int g){
	if(i>=n) return (g==1)?1:0;
	if(dp[i][g] != -1) return dp[i][g];
	if(g==1) return dp[i][g] = exp(2ll, (ll)(n-i));
	// Take the element
	ll m1 = solveutil(i+1, gcd(g, aa[i]));
	// Not take the element
	ll m2 = solveutil(i+1, g);
	return dp[i][g] = m1 + m2;
}
