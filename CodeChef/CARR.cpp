...
#define sub(x, y) ((x % MOD - y % MOD + MOD) % MOD)
...
void matMul(ll (&A)[2][2], ll (&B)[2][2]){
	ll a = add(mul(A[0][0],B[0][0]), mul(A[0][1], B[1][0]));
	ll b = add(mul(A[0][0],B[0][1]), mul(A[0][1], B[1][1]));
	ll c = add(mul(A[1][0],B[0][0]), mul(A[1][1], B[1][0]));
	ll d = add(mul(A[1][0],B[0][1]), mul(A[1][1], B[1][1]));
	A[0][0]=a,A[0][1]=b,A[1][0]=c,A[1][1]=d;
}

// Multiply matrix A by vector x: Ax=b
void matMul(ll (&A)[2][2], ll (&x)[2], ll (&b)[2]){
	ll a = add(mul(A[0][0],x[0]), mul(A[0][1], x[1]));
	ll c = add(mul(A[1][0],x[0]), mul(A[1][1], x[1]));
	b[0]=a,b[1]=c;
}

// Raise a matrix P to the power n
void matPow(ll (&P)[2][2], ll n){
	ll ret[2][2] = {{1, 0}, {0, 1}};
	while(n){
		if(n&1) matMul(ret, P);
		matMul(P, P);
	...
	P[0][0]=ret[0][0],P[0][1]=ret[0][1],P[1][0]=ret[1][0],P[1][1]=ret[1][1];
}

// Compute F_n = P^n-1 * F_1
ll solveutil(ll n, ll m){
	ll F[2]={m, 0};
	ll P[2][2]={{sub(m,1ll), sub(m,1ll)}, {1, 0}};
	matPow(P, n-1);
	matMul(P, F, F);
	return add(F[0], F[1]);
}
...
ll solve(){
	ll n,m; in,n,m; m%=MOD;
	if(n < 3) return exp(m,n);
	else return solveutil(n, m);
}
