...
public:
	int b,e; ll s;
	Data(int b, int e, ll s) : b(b),e(e),s(s) { }
};

auto cmp = [](const Data & lhs, const Data & rhs){ return lhs.s > rhs.s; };
...
void solve(){
	...
	for(int i=0;i<n;i++){
		mxh+=aa[i];
		if(mxf<=mxh) ss.push_back(Data(b, (e=i), (mxf=mxh)));
		if(mxh<0) mxh=0,b=i+1;
	}
	ll mx=mxf;
	sort(ss.begin(), ss.end(), cmp);
	for(int i=0;i<ss.size()&&ss[i].s>=mxf;i++)
		for(int j=ss[i].b;j<=ss[i].e&&ss[i].e-ss[i].b>1;j++)
			mx = ::max(mx, ss[i].s - aa[j]);
	out,mx,NL;
}
