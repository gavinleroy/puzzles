...
auto cmp = [](const PII & lhs, const PII & rhs){ return lhs.S > rhs.S; };

void dijkstra(int s, int & low){
	bool vv[100001]={};
	priority_queue<PII, VPII, decltype(cmp)> qq(cmp);
	qq.push(MP(s, 0));
	while(!qq.empty()){
		int v = qq.top().F, d = qq.top().S; qq.pop();
		...
		if(kk.find(v) != kk.end() && v != s) low=::min(low,d);

		for(int i=0;i<aa[v].size();i++){
			int vn = aa[v][i].F, dn = d+aa[v][i].S;
			if(!vv[vn]) qq.push(MP(vn, dn));		
		}
	}
}
...
void solve(){
	int low=~(1<<31);
	for(auto itr=kk.begin();itr!=kk.end();itr++) dijkstra(*itr, low);
	out,low,NL;
}
...
int main(){
	...
	for(int i=0,u,v,z;i<m;i++){
		in,u,v,z;
		aa[u].push_back(MP(v, z));
		aa[v].push_back(MP(u, z));
	}
	solve();
	return 0;
}
