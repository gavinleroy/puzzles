...
void solve(int m, int n){
	...
	for(int i = 1; i <= n; i++){
		int temp; 
		cin >> temp;
		auto search = mm.find(temp);
		if(search == mm.end()){
			vector<int> vv;
			vv.push_back(i);
			mm.insert(make_pair(temp, vv));
		}else search->S.push_back(i);
	}
	for(auto val : mm){
		if(val.F >= m) continue;
		auto search = mm.find(m - val.F);	
		if(search != mm.end()){
			int ind = 0;
			while(ind < search->S.size() && search->S[ind] == val.S[0]) ind++;
			cout << min(val.S[0], search->S[ind]) << " " << max(val.S[0], search->S[ind]) << endl;
			break;
		}// No else statement assumes that there is always an answert.
	}
}
