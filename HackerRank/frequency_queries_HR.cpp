...
/* The variable 'm' in this solution tracks the max frequency that has been reached.
 * and if the queried frequency is greater than the max recorded then a 0 is automatically returned.
 *
 * This is what pushed my solution under the time limit for test case #10.
 * I believe that it is ridiculous that something like that is needed in order to get AC
 * however it is what it is.
 */
void solve(int n){
	map<int, int> aa, tt;
	int t, v, m = 0; //This variable m tracks the highest recorded frequency.
	while(n--){
		cin >> t >> v;
		...
		if(t == 1){
			...
			if(v > m){ cout << 0 << endl; continue; }
			cout << ((tt[v] > 0) ? 1 : 0 ) << endl;
		}else throw "Value of query not recognized\n";
	}
}
