...
#endif
	int n,k,aa[1000001]; in,n,k;
	for(int i=0;i<n;i++)in,aa[i];
	
	int i=0,j,end=k-1,plants=0;
	bool check_end=false,fail=false;
	while(i<n-k && !check_end && !fail){
		for(j=end;j>=i;j--) if(aa[j]) break;
		if(j<i) fail=true;
		plants++, i=j+1, end=j+2*k-1;
		if(end >= n && i <= n-k) check_end=true;
	}

	if(!fail && check_end){
		bool good = false;
		for(i=n-1;i>=n-k&&!good;i--)good|=aa[i];
		fail = !good;
		plants++;
	}
