...
#define COST(A, B) (abs(A[0][0]-B[0][0])+abs(A[0][1]-B[0][1])+abs(A[0][2]-B[0][2])+abs(A[1][0]-B[1][0])+abs(A[1][1]-B[1][1])+abs(A[1][2]-B[1][2])+abs(A[2][0]-B[2][0])+abs(A[2][1]-B[2][1])+abs(A[2][2]-B[2][2])) 

void solve(){
	int minCost = 100;
	for(int i=0;i<8;i++){
		int cost = COST(ss, possibilities[i]);
...
#endif
	// Read in S
	for(int i=0;i<3;i++) in,ss[i][0],ss[i][1],ss[i][2];
