...
// @param f: 
// 0 if previous was not flipped. 
// 1 if previous was flipped.
// 2 if cannot be flipped.
int solver(char * ss, int ind, int f){
	if(!ss[ind]) return (f==0)?NINF:0;
	if(cache[ind][f] != INIT) return cache[ind][f];
	int m1,m2=-1;
	if(f == 0){
		m1 = (ss[ind]=='R') + solver(ss, ind+1, 0);	
		m2 = (ss[ind]=='K') + solver(ss, ind+1, 1);	
	}else if(f == 1){
		m1 = (ss[ind]=='K') + solver(ss, ind+1, 1);	
		m2 = (ss[ind]=='R') + solver(ss, ind+1, 2);	
	}else m1 = (ss[ind]=='R') + solver(ss, ind+1, 2);
	return cache[ind][f] = ::max(m1, m2);
}

void solve(char * ss){
	for(int i=0;ss[i];i++) cache[i][0]=cache[i][1]=cache[i][2]=INIT;
	out,solver(ss, 0, 0),NL;
}
