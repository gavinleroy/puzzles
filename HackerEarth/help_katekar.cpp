...
void solve(){
	...
	for(int i=0;i<n;i++){
		in,a;
		maxf=::max(maxf, ++freqs[a]);
	}
	int i=1,dp[freqs.size()+1][maxf+1];
	memset(dp, 0, sizeof dp);
	for(auto itr=freqs.begin();itr!=freqs.end();itr++,i++){
		for(int j=1;j<=maxf;j++){
			if(j<=itr->S)dp[i][j]=dp[i-1][j-1] + j;
			else dp[i][j]=::max(dp[i-1][j], dp[i][j-1]);
		}
	}
	out,dp[freqs.size()][maxf],NL;
}
