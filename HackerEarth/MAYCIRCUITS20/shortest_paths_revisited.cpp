...
bool visited[1800000];
ll dist[1800000];

struct cmp { bool operator()(const pair<int,ll> & lh, const pair<int,ll> & rh){ return lh.S > rh.S; } };

/*
 * Augment graph G to have n * k verticies. 
 * Edges from a -> b have weight w
 * Edges from a -> b+n have weight 0
 * Edges from a+n -> b+n have weight w
 * */
void solve(){
	fill(visited, visited + n * (k+1), false);
	fill(dist, dist + n * (k+1), INF);
	priority_queue< pair<int,ll>, vector<pair<int,ll> >, cmp> qq;
	qq.push(MP(0, 0ll)); dist[0] = 0ll;
	while(!qq.empty()){
		pair<int, ll> top = qq.top(); qq.pop();
		int v = top.F;
		...
		for(int i=0;i<G[v].size();i++){
			int nv = G[v][i].v + ((top.F / n) * n); 
			ll ww = G[v][i].w;
			if(dist[nv] > top.S + ww){
				dist[nv] = top.S + ww;
				qq.push(MP(nv, dist[nv]));
			}
			nv += n; 
			if(top.F / n < k && dist[nv] > top.S){
				dist[nv] = top.S;
				qq.push(MP(nv, dist[nv]));
			}
		}
	...
	for(int i=0;i<n;i++){ // Print solution
		ll m = INF;
		for(int j = 0; j <= k; j++) m = ::min(m, dist[i + n*j]);
		out,m,' ';
	}
	out,NL;
}
...
int main(){
	...
	for(int i=0;i<m;i++){
		in,u,v,w;
		u--; v--; // Make range [0,n)
		G[u].push_back(edge(v, w)); // Add verticies into adjacency list
