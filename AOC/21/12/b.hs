...
notsmall :: Vertex -> Bool
notsmall (Small _) = False
notsmall _         = True

canVisit :: S.MultiSet Vertex -> Vertex -> Bool
canVisit _ Start       = False
canVisit _ End         = True
canVisit _ (Large _)   = True
canVisit s v@(Small _)
  = (S.notMember v s) ||
    (S.foldOccur (\v' oc acc ->
                    acc && ((notsmall v') || (oc == 1))) True s)

findPaths :: Graph -> S.MultiSet Vertex -> Vertex -> Vertex -> Int
