...
(struct monkey (items M op t? next)
  #:transparent)
...
(define (parse-monkey ls)
  ...
  (define get-next
    (let* ([last-n (lambda (s) (string->number (last (string-split s))))]
      ...
      (lambda (b) (if b nt nf))))
  (monkey start-items M operation test? get-next))
    ...
    (let ([inspected (make-vector (vector-length monkeys))])
      (values (lambda (i)
              ...
              (lambda ()
                ...
                (* (vector-ref vp 0)
                   (vector-ref vp 1))))))
  (define mod (apply * (vector->list (vector-map monkey-M monkeys))))
  (for ([_ (in-range 10000)])
    (for ([(mnk i) (in-indexed monkeys)])
      (for ([item (in-list (monkey-items mnk))])
        (incr i)
        (define value (modulo ((monkey-op mnk) item) mod))
