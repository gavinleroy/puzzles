...
let solve filled =
  ...
  let max_col = S.fold (fun pn m -> Posn.fst pn |> max m) filled Int.min_int in
  let min_col = S.fold (fun pn m -> Posn.fst pn |> min m) filled Int.max_int in
  let rec drop_sand curr_p s =
    let r = Posn.snd curr_p in
    let c = Posn.fst curr_p in
    let down = Posn.down curr_p in
    let left = Posn.diag_left curr_p in
    let right = Posn.diag_right curr_p in
    if max_row < r || c < min_col || max_col < c then
      None
    else if not(S.mem down s) then
      drop_sand down s
    else if not(S.mem left s) then
      drop_sand left s
    else if not(S.mem right s) then
      drop_sand right s
    else
