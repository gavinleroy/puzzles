...
(defun next-secret (sn)
  (declare (type (integer 0 16777215) sn))
  (macrolet ((mix-prune (n)
               `(setf sn (mod (logxor ,n sn) 16777216))))
    (progn (mix-prune (* sn 64))
           (mix-prune (floor sn 32))
           (mix-prune (* sn 2048))
           sn)))

(defun goto (sn times)
  (dotimes (_ times)
    do (setf sn (next-secret sn)))
  sn)

(defun part-1 (prices)
  (loop for sn in prices
        sum (goto sn 2000)))
...
(defun banana-prices (sn times)
  (loop with diffs = (fset:empty-seq) and prices = (fset:empty-bag)
        for i from 0 below times
        for prev-price = (mod sn 10) then price
        for curr = (next-secret sn) then (next-secret curr)
        for price = (mod curr 10)
        for diff = (- price prev-price)
        do (fset:push-last diffs diff)
        unless (or (<= i 2) (fset:member? diffs prices))
          do (fset:includef prices diffs price)
        when (< 2 i)
          do (fset:pop-first diffs)
...
(compose best-price
         #'fset:greatest
         #'fset:range
         (lambda (bag) (fset:convert 'fset:map bag)))

(defun part-2 (lines)
  (best-price
   (reduce (lambda (acc sn) (fset:bag-sum acc (banana-prices sn 2000)))
           lines :initial-value (fset:empty-bag))))

(defun main ()
  (let ((lines (mapcar #'parse-integer (uiop:read-file-lines "input.data"))))
    (format t "Part 1: ~a~%" (part-1 lines))
    (format t "Part 2: ~a~%" (part-2 lines))))

#+nil ;; TODO optimize, part 2 takes ~15s
(time (main))
