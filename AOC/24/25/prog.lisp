...
(defun read-input (filename)
  (loop for block in (ppcre:split "\\n\\n" (uiop:read-file-string filename))
        for reduced = (loop with acc fixnum = 0
                            for c across block
                            unless (char= c #\Newline)
                              do (setf acc (ash acc 1))
                            when (char= c #\#)
                              do (setf acc (logior acc 1))
                            finally (return acc))
        if (char= (char block 0) #\#)
          collect reduced into locks
        else
          collect reduced into keys
        finally (return (values keys locks))))

(defun part-1 ()
  (multiple-value-bind (keys locks) (read-input "input.data")
    (let ((count 0))
      (dolist (a keys)
        (dolist (b locks)
          (when (zerop (logand a b))
            (incf count))))
      count)))
