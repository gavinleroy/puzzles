...
#define sorted(aa, sr, st) for(int i = sr; i < st-1; i++){if(aa[i]>aa[i+1]){srt=false;break;}} 

int solve(int *aa, int start, int stop){
	bl srt = true;
	sorted(aa, start, stop)
	if(srt) 
		return stop-start;
	else{
		int f1 = solve(aa, start, start+(stop-start)/2);
		int f2 = solve(aa, start+(stop-start)/2, stop);
		return ::max(f1,f2);
	}
}
