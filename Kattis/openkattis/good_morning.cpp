...
pair<int,int> dial[10] = {MP(3,1),MP(0,0),MP(0,1),MP(0,2),MP(1,0),MP(1,1),MP(1,2),MP(2,0),MP(2,1),MP(2,2)};

bool check(int n){
	string s=to_string(n);
	pair<int,int>p1,p2;
	for(int i=0;i<s.size()-1;i++){
		p1=dial[s[i]-'0'],p2=dial[s[i+1]-'0'];	
		if(p2.R < p1.R || p2.C < p1.C) return false;
	}
	return true;
}
