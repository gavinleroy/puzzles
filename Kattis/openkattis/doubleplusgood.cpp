...
int main(){
	...
	set<ll> ss;
	ll t;
	while(in,t) vv.push_back(t);
	for(int i=0;i<(1<<(vv.size()-1));i++){
		int ops = i;
		string ls=to_string(vv[0]);
		ll total=0;
		for(int k=1;k<vv.size();k++,ops>>=1){
			if(ops&1) ls+=to_string(vv[k]);
			else{
				total += stoll(ls);
				ls = to_string(vv[k]);
			}
		}
		ss.insert(total + stoll(ls));
	}
	out,ss.size(),NL;
	return 0;
}
