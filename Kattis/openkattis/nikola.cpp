...
int aa[1001],dp[1001][1001],n,ans=INIT;

void solve(int i, int p, int c){
	if(i==n-1){ dp[i][p]=c;ans=::min(ans,c); return; }
	if(dp[i][p] < c || c >= ans) return;
	dp[i][p]=c;
	if(i-p >= 0) solve(i-p, p, aa[i-p] + c);
	if(i+p+1 < n) solve(i+p+1, p+1, aa[i+p+1]+c);
}
