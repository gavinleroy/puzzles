...
#define B '*'
#define W '.'
#define valid(x,y) (x>=0&&x<3&&y>=0&&y<3)

using namespace std;

map<int, int> cache;
char initial[3][3], goal[3][3] = {{W,W,W},{W,W,W},{W,W,W}};
int rr[4] = {-1, 1, 0, 0};
int cc[4] = {0, 0, -1, 1};
...
public:
	int dist;
	State(char bb[3][3], int r, int c, int d) :
		pr(r),pc(c),dist(d)	
	{ 
		for(int i=0;i<3;i++){
	...
	bool push(char nb[3][3], int r, int c){
		if(r==pr && c==pc) return false;
		for(int i=0;i<3;i++){
			for(int j=0;j<3;j++) nb[i][j]=board[i][j];
		}	
		nb[r][c] = (board[r][c]==W)?B:W;
		for(int i=0;i<4;i++){
			int nr = r+rr[i], nc=c+cc[i];
			if(valid(nr, nc)){
				nb[nr][nc] = (board[nr][nc]==W)?B:W;
			}
		}
		return true;
	}
	...
	bool check(){
		for(int i=0;i<3;i++){
			for(int j=0;j<3;j++) if(goal[i][j]!=board[i][j]) return false;
		}
		return true;
	}
	...
	void print(){
		for(int i=0;i<3;i++){
			for(int j=0;j<3;j++) out,board[i][j];
			out,NL;
		}
		out,NL;
	 }
...
void solve(){
	int ans=0;
	queue<State> qq;	
	State init(initial, -1, -1, 0);
	if(init.check()) goto done;
	qq.push(init);
	while(!qq.empty()){
		State s = qq.front(); qq.pop();
		for(int i=0;i<3;i++){
			for(int j=0;j<3;j++){
				char nb[3][3];
				if(s.push(nb, i, j)){
					State ns(nb, i, j, s.dist+1);
					if(ns.check()){
						ans = ns.dist;
						goto done;
					}else qq.push(ns);
...
int main(){
	...
	for(int t=in;t--;){
		for(int i=0;i<3;i++){
			for(int j=0;j<3;j++) in,initial[i][j];
		}	
		solve();
	}
	return 0;
