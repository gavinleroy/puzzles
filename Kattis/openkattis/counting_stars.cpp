...
public:
	...
	int count_stars(char * dd){
		int ans=0;
		for(int i=0;i<n;i++){
			if(parent[i]==i && dd[i]=='-') ans++;
		}
		return ans;
	}
};
...
#endif
	int height,width,sc=1;
	char c, *aa;
	while(in,height,width){
		DisjointSet ds(height*width);	
		aa = new char[height*width];
		for(int i=0;i<height;i++){
			for(int j=0;j<width;j++){
				in,c; aa[i*width+j]=c;
				if(c=='#') continue;
				int ci=i*width+j;
				int tt[2] = {i*width+j-1,(i-1)*width+j};
				for(int k=(!j)?1:0;k<2;k++){
					if(tt[k]>=0&&aa[tt[k]]==c) ds.join(ci, tt[k]);
				}
			}
		}
		out,"Case ",sc++,": ",ds.count_stars(aa),NL;
		delete []aa;
	}
	return 0;
}
