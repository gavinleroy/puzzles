...
int hh[20001],kk[20001];

void solve(int n, int m){
	int nt=n,ans=0;
	sort(hh,hh+n);
	sort(kk,kk+m);
	for(int i=0,k=0;i<nt&&k<m;i++,k++){
		if(kk[k]>=hh[i])ans+=kk[k],n--;
		else i--;
	}
	if(n) out,"Loowater is doomed!\n";
	else out,ans,NL;
}
...
int main(){
	...
	while(in,n,m){
		if(!n&&!m)break;
		for(int i=0;i<n;i++) in,hh[i];
		for(int i=0;i<m;i++) in,kk[i];
		if(m<n) out,"Loowater is doomed!\n";
		else solve(n,m);
	}
	return 0;
}
